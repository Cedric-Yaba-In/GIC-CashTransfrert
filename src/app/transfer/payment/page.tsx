...
export default function PaymentPage() {
  ...
  const [selectedReceiverMethod, setSelectedReceiverMethod] = useState<any>(null)
  const [loading, setLoading] = useState(false)
  const [step, setStep] = useState(1)
  const [receiverPaymentMethodId, setReceiverPaymentMethodId] = useState<string>('')
  const [flutterwaveOptions, setFlutterwaveOptions] = useState<any[]>([])
  const [selectedFlutterwaveOption, setSelectedFlutterwaveOption] = useState<string>('')
  ...
  useEffect(() => {
    ...
    // Charger les méthodes de réception avec le montant que le destinataire recevra
    const receivedAmount = data.feeCalculation?.summary?.amountReceived || data.amount
    fetchReceiverMethods(data.receiverCountryId, receivedAmount)
  }, [])
  ...
  const fetchReceiverMethods = async (receiverCountryId: string, amount: number) => {
    try {
      console.log('Fetching receiver methods for:', { receiverCountryId, amount })
      const response = await fetch(`/api/transfer/receiver-methods?receiverCountryId=${receiverCountryId}&amount=${amount}`)
      const data = await response.json()
      
      console.log('Receiver methods response:', data)
      
      if (response.ok) {
        const methods = Array.isArray(data) ? data : []
        setReceiverPaymentMethods(methods)
        console.log('Set receiver methods:', methods)
      } else {
  ...
  const onSubmit = async (data: PaymentForm) => {
    ...
    try {
      ...
      const response = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({
          ...transferData,
          senderPaymentMethodId: data.paymentMethodId,
          receiverPaymentMethodId: receiverPaymentMethodId,
          receiverSubMethod: selectedFlutterwaveOption || null,
          ...
          // Sauvegarder les informations de réception pour l'admin
          adminNotes: JSON.stringify({
            receiverPaymentMethod: selectedReceiverMethod?.paymentMethodName,
            flutterwaveOption: selectedFlutterwaveOption,
            receiverPaymentInfo: data.receiverPaymentInfo,
            senderPaymentInfo: data.senderPaymentInfo
          })
        }),
      })
      ...
      if (response.ok) {
        // Si c'est une méthode Flutterwave, traiter le paiement
        if (selectedPaymentMethod?.paymentMethodType === 'FLUTTERWAVE') {
          const paymentResponse = await fetch('/api/flutterwave/process-payment', {
            method: 'POST',
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200">
              ...
              <div className="space-y-2">
                ...
                {transferData.feeCalculation && (
                  <>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-600 text-sm">Frais totaux:</span>
                      <span className="font-semibold text-red-600">-{Number(transferData.feeCalculation.fees?.total?.amount || 0).toFixed(2)} {transferData.feeCalculation.senderCurrency}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2 border-t border-purple-200">
                      <span className="text-purple-700 font-medium">Vous payez:</span>
                      <span className="text-xl font-bold text-purple-600">
                        {Number(transferData.feeCalculation.summary?.totalPaid || 0).toFixed(2)} {transferData.feeCalculation.senderCurrency}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-green-700 font-medium">Destinataire reçoit:</span>
                      <span className="text-lg font-bold text-green-600">
                        {Number(transferData.feeCalculation.summary?.amountReceived || 0).toFixed(2)} {transferData.feeCalculation.receiverCurrency}
                      </span>
                    </div>
                    {transferData.feeCalculation.exchange && transferData.feeCalculation.senderCurrency !== transferData.feeCalculation.receiverCurrency && (
                      <div className="text-xs text-gray-500 pt-1 border-t border-purple-100">
                        Taux: 1 {transferData.feeCalculation.senderCurrency} = {Number(transferData.feeCalculation.exchange.appliedRate || 0).toFixed(4)} {transferData.feeCalculation.receiverCurrency}
                      </div>
                    )}
                  </>
                )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8">
              ...
              <div className="space-y-8 p-8">
                ...
                <div className="bg-gradient-to-r from-teal-50 to-cyan-50 rounded-2xl p-8 border border-teal-100">
                  <div className="flex items-center mb-6">
                    ...
                    <div className="ml-4">
                      <h3 className="text-xl font-bold text-gray-900">Méthodes de réception disponibles</h3>
                      <p className="text-gray-600">Sélectionnez comment le destinataire recevra l'argent</p>
                    </div>
                  </div>
                  
                  {receiverPaymentMethods.length > 0 ? (
                    <div className="grid gap-4">
                      {receiverPaymentMethods.map((method, index) => {
                        const gradients = [
                        ...
                        return (
                          <div
                            ...
                            className={`border-2 rounded-xl p-6 cursor-pointer transition-all duration-200 ${
                              receiverPaymentMethodId === method.paymentMethodId
                                ? 'border-teal-400 bg-teal-50 shadow-lg'
                                : 'border-gray-200 hover:border-teal-300 hover:bg-teal-50'
                            }`}
                            onClick={() => {
                              setReceiverPaymentMethodId(method.paymentMethodId)
                              setSelectedReceiverMethod(method)
                            }}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                <div className={`bg-gradient-to-r ${gradients[index % 3]} p-3 rounded-xl mr-4`}>
                                  <CreditCard className="h-6 w-6 text-white" />
                                </div>
                                <div>
                                  <h4 className="text-lg font-bold text-gray-900 mb-1">{method.paymentMethodName}</h4>
                                  <div className="flex items-center space-x-3">
                                    ...
                                    <span className="text-sm text-gray-600">
                                      Solde: <span className="font-semibold">{method.balance.toFixed(2)} {method.currencyCode}</span>
                                    </span>
                                  </div>
                    ...
                    <div className="text-center py-8 bg-yellow-50 rounded-xl border border-yellow-200">
                      ...
                      <p className="text-yellow-700 font-medium">Aucune méthode de réception disponible</p>
                      <p className="text-yellow-600 text-sm">Le pays de destination n'a pas suffisamment de fonds</p>
                    </div>
                  )}
                </div>
            ...
            {step === 3 && selectedPaymentMethod && (
              <div className="space-y-8 p-8">
                ...
                {selectedReceiverMethod?.paymentMethodType === 'FLUTTERWAVE' && (
                  <div className="bg-blue-50 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-blue-900">Transfert via Flutterwave</p>
                          <p className="text-sm text-blue-700">Transfert automatique après validation administrative</p>
                        </div>
                      </div>
                      
                      {/* Sélection de l'option Flutterwave */}
                      <div className="bg-white rounded-lg p-4 border border-blue-200">
                        <h4 className="font-medium text-blue-900 mb-3">Choisissez le mode de réception :</h4>
                        <div className="space-y-3">
                          <div 
                            className={`border-2 rounded-lg p-3 cursor-pointer transition-all ${
                              selectedFlutterwaveOption === 'mobilemoney' 
                                ? 'border-blue-500 bg-blue-50' 
                                : 'border-gray-200 hover:border-blue-300'
                            }`}
                            onClick={() => setSelectedFlutterwaveOption('mobilemoney')}
                          >
                            <div className="flex items-center">
                          ...
                          <div 
                            ...
                            onClick={() => setSelectedFlutterwaveOption('banktransfer')}
                          >
                            <div className="flex items-center">
                              <div className={`w-4 h-4 rounded-full border-2 mr-3 ${
                                selectedFlutterwaveOption === 'banktransfer'
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600">Vers un compte bancaire</p>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {selectedFlutterwaveOption === 'mobilemoney' && (
                        <div className="bg-white rounded-lg p-4 border border-green-200">
                          <h4 className="font-medium text-green-900 mb-3">Informations Mobile Money</h4>
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Numéro Mobile Money du destinataire *
                            </label>
                ...
                {/* Informations d'envoi selon la méthode choisie */}
                <div className="bg-blue-50 rounded-xl p-6 mb-6">
                  <h3 className="font-semibold text-blue-900 mb-4">Informations d'envoi - {selectedPaymentMethod.paymentMethodName}</h3>
                  ...
                  {selectedPaymentMethod.paymentMethodType === 'FLUTTERWAVE' && (
                    <div className="space-y-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-blue-900">Paiement sécurisé Flutterwave</p>
                          <p className="text-sm text-blue-700">Vous serez redirigé vers Flutterwave pour finaliser le paiement</p>
                        </div>
                      </div>
                    </div>
                  )}
